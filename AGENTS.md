# Agent instructions — IPTV Tunerr

Instructions for AI agents and maintainers. **This repo is IPTV Tunerr** (IPTV→Plex bridge); it also uses the agentic template workflow (memory-bank, verify, docs). Main app: [cmd/iptv-tunerr](cmd/iptv-tunerr/main.go). Navigation: [memory-bank/repo_map.md](memory-bank/repo_map.md).
