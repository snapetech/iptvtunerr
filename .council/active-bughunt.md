...
internal/plexlabelproxy/proxy.go:602:	if !p.canElevate(req, elevationToken) {
internal/plexlabelproxy/proxy.go:806:func inboundPlexToken(req *http.Request) string {
internal/plexlabelproxy/proxy.go:813:	if _, hopByHopToken := connectionHeaderNames(req.Header)["x-plex-token"]; hopByHopToken {
internal/plexlabelproxy/proxy.go:819:func connectionHeaderNames(h http.Header) map[string]struct{} {
internal/plexlabelproxy/proxy.go:942:	if p.blockBypassAllowed(req, inboundPlexToken(req)) {
internal/plexlabelproxy/proxy.go:948:func (p *Proxy) blockBypassAllowed(req *http.Request, token string) bool {
internal/plexlabelproxy/proxy.go:978:	if ip := trustedCloudflareConnectingIP(req); ip != "" {
internal/plexlabelproxy/proxy.go:981:	if ip := closestTrustedForwardedFor(req); ip != "" {
internal/plexlabelproxy/proxy.go:987:func closestTrustedForwardedFor(req *http.Request) string {
internal/plexlabelproxy/proxy.go:1001:func trustedCloudflareConnectingIP(req *http.Request) string {
internal/plexlabelproxy/proxy.go:1006:	closest := closestTrustedForwardedFor(req)
...
internal/plexlabelproxy/proxy.go:656:		p.sessions = make(map[string]sessionRecord)
internal/plexlabelproxy/proxy.go:659:	rec := sessionRecord{userToken: originalToken, sourceKey: source, expiresAt: time.Now().Add(elevatedSessionTTL)}
...
internal/plexlabelproxy/proxy.go:750:		if strings.TrimSpace(rec.userToken) != "" && sessionRecordMatchesSource(rec, req) {
internal/plexlabelproxy/proxy.go:764:func sessionRecordMatchesSource(rec sessionRecord, req *http.Request) bool {
internal/plexlabelproxy/proxy.go:1302:	keys := sessionCorrelationKeys(req)
internal/plexlabelproxy/proxy.go:1324:		if !sessionRecordMatchesSource(rec, req) {
internal/plexlabelproxy/proxy.go:1342:			p.logPlaybackCorrelation(req, "source_mismatch", sourceMismatch, keys)
...
internal/plexlabelproxy/proxy.go:1402:		if !sessionRecordMatchesSource(rec, req) {
internal/plexlabelproxy/proxy_test.go:1811:	if !strings.Contains(logBuf.String(), "outcome=source_mismatch") {
internal/plexlabelproxy/proxy_test.go:1812:		t.Fatalf("expected source_mismatch playback log, got %s", logBuf.String())
...
internal/plexlabelproxy/proxy.go:500:	if scope == scopeNone && !p.shouldRewriteTunerEntitlement(resp) {
internal/plexlabelproxy/proxy.go:518:		if p.shouldRewriteTunerEntitlement(resp) {
internal/plexlabelproxy/proxy.go:519:			return restoreBody(resp, RewriteTunerEntitlementFlags(body), encoding)
internal/plexlabelproxy/proxy.go:530:	if p.shouldRewriteTunerEntitlement(resp) {
internal/plexlabelproxy/proxy.go:531:		rewritten = RewriteTunerEntitlementFlags(rewritten)
internal/plexlabelproxy/proxy.go:1498:func (p *Proxy) shouldRewriteTunerEntitlement(resp *http.Response) bool {
