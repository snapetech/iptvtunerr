...
- Hardened `.github/workflows/publish-chocolatey.yml` so the `Pack and push` step rewrites the full nuspec and install script at runtime before packing. This lets an existing tag such as `v0.1.85` be republished with corrected Chocolatey metadata even though the tag itself predates the packaging fix.
...
- Headless checks confirmed the current package page still lists `0.1.68` as `Waiting for Maintainer`, while the `v0.1.85` workflow now gets through checkout, setup-dotnet, tag guard, asset download, and package creation before Chocolatey returns `403 Forbidden` at `https://push.chocolatey.org/api/v2/package/`.
- Attempted the exact-version `v0.1.68` moderation update, which is the Chocolatey-documented path for changes during moderation, but the old checkout's tag guard predated ancestor mode and rejected the older tag before push. The Chocolatey workflow now performs the ancestor guard inline from the current workflow file so old moderation tags can be republished.
- The next exact-version `v0.1.68` dispatch pushed successfully, but raw-package inspection showed `dotnet pack` had rewritten the root nuspec and dropped `packageSourceUrl`, `projectSourceUrl`, `docsUrl`, and `bugTrackerUrl`. Patched the workflow to replace the packed root nuspec with the exact Chocolatey nuspec and assert all moderation metadata tags before push.
- The first packed-nuspec preservation dispatch hung inside the monolithic pack/push script before any native command output reached the log. Patched the workflow to use a unique temporary pack directory, emit progress markers, and bound `dotnet pack`/`dotnet nuget push` with `timeout`.
- Updated the release-channel guide and changelog with the moderation state and remediation.
- Verification: `xmllint` on the nuspec and SVG, PowerShell parser checks, extracted workflow parser checks after replacing GitHub expressions, simulated `Pack and push` dry run against old `v0.1.85` tag contents, local packed-nupkg metadata inspection, local `dotnet pack` smoke for the Chocolatey nuspec, install-script shim-path stub test, `git diff --check`, local identity scan, and full `./scripts/verify` passed. `scripts/verify` regenerated tracked council scan files.
