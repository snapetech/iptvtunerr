...
- **Emby registration state is private and symlink-aware:** persisted Emby registration IDs and tuner/XMLTV URLs now write through private randomized temp files and refuse symlinked state targets.
- **Recorded catchup publishing is file-only and symlink-aware:** recorded media publishing now refuses symlinked or directory destinations, writes copied media through private temp files, and keeps published item directories private.
- **Catchup recorder spool files are private and symlink-aware:** resilient catchup capture now creates private lane directories and media artifacts while refusing symlinked partial spool targets during full and resumed writes.
- **Runtime state writes are private and symlink-aware:** autopilot profile decisions and Plex-label-proxy abuse/block state now use private directories/files, randomized temp writes, and symlink overwrite refusal.
- **Shared provider-account lease files are private and symlink-aware:** cross-process provider pooling now keeps lease directories, lease JSON, and lock files private on disk while refusing symlinked lease/lock targets.
- **Provider EPG disk cache writes are private and symlink-aware:** XMLTV disk cache bodies and validator metadata now write through private temp files, keep `0600` file modes, and refuse symlinked cache targets.
- **VOD materializer cache writes are private and symlink-aware:** materialized provider media now uses private cache directories/files and refuses to overwrite symlinked partial/download destinations.
- **Diagnostic harness script execution is path-contained:** operator diagnostics now reject script names that escape the repo `scripts/` directory before launching the bounded harness process.
- **WebUI logo storage is contained on disk:** logo uploads now normalize unsafe filenames, write through private temp files, keep the logo directory and files private, and refuse to serve symlinked or path-escaped stored logo entries.
- **Plex API response diagnostics redact echoed secrets:** Plex registration, library, user, and sharing helpers now scrub response body previews before logging or returning error text.
- **Successful HLS startup logs redact first segment URLs:** live HLS relay logs now scrub credential-bearing absolute media segment URLs before recording the `first-seg` diagnostic field.
- **Runtime XMLTV startup logs redact credentials:** `run` and `serve` mode logs now keep useful guide URL context while stripping userinfo and query secrets from external XMLTV URLs.
- **Runtime snapshots redact credential-bearing URLs:** operator runtime snapshots now scrub provider, guide, HDHR, and public stream URL fields before storing or exposing diagnostic state.
- **Cookie-import dry runs no longer echo cookie values:** `iptv-tunerr import-cookies --dry-run` now reports domains and cookie names only, with values redacted.
- **Evidence intake bundles are private by default:** operator evidence scaffolds now create the case directory and sensitive subdirectories with `0700` permissions while keeping generated notes/readme files private.
- **Stream debug tee evidence is now private on disk:** when `IPTV_TUNERR_DEBUG_TEE_BYTES` is enabled, captured stream byte samples are written under a `0700` directory with `0600` files instead of inheriting broadly readable defaults.
- **Stream-attempt diagnostics redact segment and error payload secrets:** `/debug/stream-attempts.json` now scrubs first-segment URLs plus upstream/final error text before persisting or returning attempt history, closing a leak path where provider query tokens could survive outside the already-redacted URL fields.
- **Virtual-channel recovery state no longer persists provider secrets:** recovery history now stores redacted source/fallback URLs, sanitizes legacy state on load, and writes the optional recovery state file with private permissions.
- **Event hook reports redact webhook credentials:** `/debug/event-hooks.json` now reports webhook URLs and custom headers through the same redaction boundary used for provider diagnostics, so bearer/API-key headers and credential-bearing URL fields are not reflected back in operator status output.
- **Virtual-channel playback blocks literal private upstream IPs by default:** plain and branded virtual-channel media proxying now rejects catalog movie/episode URLs pointing at loopback, RFC1918, link-local, or unspecified literal IPs before proxying or FFmpeg/content probing unless explicitly disabled for lab/private-origin testing.
- **Xtream VOD proxy blocks literal private upstream IPs by default:** `/movie/...` and `/series/...` now reject catalog VOD URLs pointing at loopback, RFC1918, link-local, or unspecified literal IPs unless explicitly disabled for lab/private-origin testing.
- **Xtream proxy path segments now round-trip safely:** generated live/movie/series URLs escape username, password, and stream IDs as path segments, while inbound proxy routes parse the escaped path instead of the decoded path to avoid reserved-character path confusion.
- **Operator diagnostics now scrub reflected provider and Plex credentials:** bounded harness stdout, diagnostic summaries, Ghost Hunter helper output, and `debug-bundle --redact` live payloads now strip credential-bearing URLs, auth headers, cookies, and token-shaped key/value pairs before they leave local tooling.
...
- **Native HLS/DASH mux segment proxying blocks literal private upstream IPs by default:** `seg=` targets pointing at loopback, RFC1918, link-local, or unspecified literal IPs now return `403 blocked_private_upstream` unless explicitly disabled for lab/private-origin testing, and the optional HLS mux demo is behind the operator locality gate.
